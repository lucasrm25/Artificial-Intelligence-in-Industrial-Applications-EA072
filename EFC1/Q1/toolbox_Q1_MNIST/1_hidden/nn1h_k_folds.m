...
[N,m] = size(X);
% n = input('Number of neurons at the hidden layer (nonlinear units) = ');
...
for fold=1:k,
    ...
    Xacc = [];Sacc = [];
    for i=1:k,
        load(fullfile(filename,sprintf('data%d',i)));
        if i~=fold,
    ...
    [w1,w2,eq,CERv,stw1,stw2,rms_w,CER_min,n_iter_v,n_iter,error_per_class_v] = init_k_folds(n_in,n,n_out,fold,resp);
    if fold == 1,
        disp(sprintf('Number of inputs = %d',m));
        np1 = n*(m+1);np2 = n_out*(n+1);
        n_weights = np1+np2;disp(sprintf('Number of weights in the neural network = %d',n_weights));
    end
    rms_w = [rms_w;qmean2(w1,w2)];
    [Ew,dEw,eqm,CER,error_per_class] = process(X,S,Xv,Sv,w1,w2);
    ...
    while m_norm(dEw) > threshold && n_iter < n_itermax,
        if success,
            p1 = reshape(p(1:np1),m+1,n)';
            p2 = reshape(p(np1+1:np1+np2),n+1,n_out)';
        ...
        mi = p'*r;
        alpha = mi/delta;
        vw = [reshape(w1',np1,1);reshape(w2',np2,1)];
        vw1 = vw + alpha*p;
        w11 = reshape(vw1(1:np1),m+1,n)';
        w21 = reshape(vw1(np1+1:np1+np2),n+1,n_out)';
        [Ew1,dEw1,eqm1,CER1,error_per_class1] = process(X,S,Xv,Sv,w11,w21);
        ...
        if comp(it2) > 0,
            ...
            dEw = dEw1;
            deltaw1 = m_norm(w1-w11);deltaw2 = m_norm(w2-w21);
            stw1 = [stw1;deltaw1];stw2 = [stw2;deltaw2];
    ...
    save(strcat('evol',sprintf('%d',fold)),'eq','CERv','stw1','stw2','rms_w','n_iter','lambda','comp','CER_min','n_iter_v','error_per_class_v');
