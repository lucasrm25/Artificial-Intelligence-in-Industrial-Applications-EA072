...
n(3,1) = input('Number of neurons at the third hidden layer (nonlinear units) = ');
...
for fold=1:k,
    ...
    [w1,w2,w3,w4,eq,CERv,stw1,stw2,stw3,stw4,rms_w,CER_min,n_iter_v,n_iter,n_itermax,error_per_class_v] = init_k_folds(n_in,n,n_out,fold,resp);
    if fold == 1,
        disp(sprintf('Number of inputs = %d',m));
        np1 = n(1)*(m+1);np2 = n(2)*(n(1)+1);np3 = n(3)*(n(2)+1);np4 = n_out*(n(3)+1);
        n_weights = np1+np2+np3+np4;disp(sprintf('Number of weights in the neural network = %d',n_weights));
    end
    rms_w = [rms_w;qmean2(w1,w2,w3,w4)];
    [Ew,dEw,eqm,CER,error_per_class] = process(X,S,Xv,Sv,w1,w2,w3,w4);
    ...
    while m_norm(dEw) > threshold & n_iter < n_itermax,
        if success,
            ...
            p4 = reshape(p(np1+np2+np3+1:n_weights),n(3)+1,n_out)';
            s = hprocess(X,S,w1,w2,w3,w4,p1,p2,p3,p4);
            delta = p'*s;
        else
        ...
        w41 = reshape(vw1(np1+np2+np3+1:n_weights),n(3)+1,n_out)';
        [Ew1,dEw1,eqm1,CER1,error_per_class1] = process(X,S,Xv,Sv,w11,w21,w31,w41);
        ...
        if comp(it2) > 0,
            ...
            if CER1 < CER_min,
                CER_min = CER1;
                w1_prov = w1;w2_prov = w2;w3_prov = w3;w4_prov = w4;
                ...
                save(strcat('w4v',sprintf('%d',fold)),'w4');
                w1 = w1_prov;w2 = w2_prov;w3 = w3_prov;w4 = w4_prov;
                n_iter_v = n_iter;
            end
            ...
            rms_w = [rms_w;qmean2(w11,w21,w31,w41)];
            w1 = w11;w2 = w21;w3 = w31;w4 = w41;
    ...
    save(strcat('w4',sprintf('%d',fold)),'w4');
    save(strcat('evol',sprintf('%d',fold)),'eq','CERv','stw1','stw2','stw3','stw4','rms_w','n_iter','lambda','comp','CER_min','n_iter_v','error_per_class_v');
